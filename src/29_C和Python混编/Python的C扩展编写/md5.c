 ...
 * @brief HMAC-MD5 计算
 * @param self
 * @param args 传入数据和密钥两个字符串
 * @return 计算结果
 */
PyObject *MD5_hmac_md5(PyObject *self, PyObject *args)
{
	char *data = NULL;
	...
	return Py_BuildValue("s", _digestToString(digest));
}
 ...
 * @return 计算结果
 */
PyObject *MD5_md5_32(PyObject *self, PyObject *args)
{
	char *data = NULL;
	Py_ssize_t len;
	...
	return Py_BuildValue("s", _digestToString(md5));
}
 ...
 * @return 计算结果
 */
PyObject *MD5_md5_16(PyObject *self, PyObject *args)
{
	char *digest32 = NULL;
	if (!PyArg_ParseTuple(args, "s", &digest32))
	{
		return NULL;
	}
	...
	return Py_BuildValue("s", digest16);
}

// 方法表
PyMethodDef MD5Methods[] =
{
	{"md5_32", MD5_md5_32, METH_VARARGS, NULL},
	{"md5_16", MD5_md5_16, METH_VARARGS, NULL},
	{"hmac_md5", MD5_hmac_md5, METH_VARARGS, NULL},
	{NULL, NULL, 0, NULL}
};
...
// 模块初始化函数
PyMODINIT_FUNC PyInit_md5(void)
{
	return PyModule_Create(&md5module);
}